// V_PRIME_PDP_35 | Unlock Bonus Free Gifts
// Forked from c-prime-pdp-25.js — ports that test's Var A ("Inline gifts
// shown") and Var C ("Bordered gift tiles with lock/unlock") in as this
// test's Var A / Var B. Every selector below targets c-pdp35-* elements
// only, so this never binds to c-prime-pdp-25.js's markup (or vice versa) if
// both are ever rendered on a page at once.
//
...
(function () {
  function getCheckedTierQuantity(root) {
    var checked = root.querySelector('.c-pdp35-row__radio:checked');
  ...
  // visually for Control — see c-prime-pdp-35.css) so gift-granting logic
  // must check this itself: Control must never actually receive a gift,
  // only shoppers c-intelligems-tests.js has bucketed into a named PDP_35
  ...
  function pdp35VariantActive() {
    return ['c-primePdp35VarA', 'c-primePdp35VarB'].some(function (cls) {
      return document.body.classList.contains(cls);
    });
  }

  // Toggles each gift card's unlocked/locked look based on the selected row's
  // quantity vs. that card's own data-pdp35-unlock-qty threshold.
  function updateGiftLocks(root) {
    var quantity = getCheckedTierQuantity(root);
    root.querySelectorAll('[data-pdp35-gift]').forEach(function (gift) {
      var unlockQty = parseInt(gift.getAttribute('data-pdp35-unlock-qty'), 10) || 0;
      gift.classList.toggle('c-pdp35-gift--unlocked', quantity >= unlockQty);
    });
  }

  function updateAtcPrice(root) {
    var checked = root.querySelector('.c-pdp35-row__radio:checked');
    var priceEl = root.querySelector('[data-pdp35-atc-price]');
    if (!checked || !priceEl) return;
    priceEl.textContent = checked.getAttribute('data-pdp35-price-money');
  }
  ...
  // c-prime-pdp-25.js for the identical pattern this is forked from).
  ...
  function generateBundleId() {
    return 'pdp35-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8);
  }
  ...
  function getUnlockedGiftItems(root, bundleId) {
    ...
    root.querySelectorAll('[data-pdp35-gift]').forEach(function (gift) {
      ...
      items.push({
        ...
        properties: {
          _pdp35_gift: 'true',
          _pdp35_gift_key: gift.getAttribute('data-pdp35-gift-key'),
          _pdp35_bundle_id: bundleId
        }
      });
    });
    return items;
  ...
  function addToCart(root, button) {
    ...
    var mainItem = {
      id: variantId,
      quantity: quantity,
      properties: { _pdp35_bundle_id: bundleId, _pdp35_min_qty: quantity }
    };
  ...
  // radios are the same unconditional markup across Var A/B, so this fires
  // identically for everyone. Tier 1 (1 book) has no event — only tiers 2/3
  // are tracked, per spec.
  ...
  function initRoot(root) {
    if (root.dataset.pdp35Init) return;
    root.dataset.pdp35Init = 'true';

    root.querySelectorAll('.c-pdp35-row__radio').forEach(function (radio) {
    ...
    var atcButton = root.querySelector('[data-pdp35-atc]');
    if (atcButton) {
  ...
  function run() {
    document.querySelectorAll('.c-pdp35-variant').forEach(initRoot);
  }
  ...
  // _pdp35_bundle_id property, which is only ever knowable by fetching
  ...
  function cartAddGift(giftKey, bundleId) {
    var gift = window.pdp35GiftVariants && window.pdp35GiftVariants[giftKey];
    ...
    return fetch(window.routes.cart_add_url + '.js', {
      ...
      body: JSON.stringify({
        items: [{
          id: gift.id,
          quantity: 1,
          properties: { _pdp35_gift: 'true', _pdp35_gift_key: giftKey, _pdp35_bundle_id: bundleId }
        }]
      })
    });
  }
  ...
  function diffBundleGifts(cart, bundleId) {
    ...
    cart.items.forEach(function (item) {
      if (!item.properties || item.properties._pdp35_bundle_id !== bundleId) return;
      if (item.properties._pdp35_gift) {
        giftsByKey[item.properties._pdp35_gift_key] = item;
      } else {
    ...
    var variantActive = pdp35VariantActive();
  ...
  function reconcileAllBundles() {
    fetchCart()
      .then(function (cart) {
        var bundleIds = [];
        cart.items.forEach(function (item) {
          var bundleId = item.properties && item.properties._pdp35_bundle_id;
  ...
  document.addEventListener('change', function (event) {
    ...
    fetchCart()
      .then(function (cart) {
        ...
        var bundleId = item && item.properties && item.properties._pdp35_bundle_id;
        if (!bundleId || item.properties._pdp35_gift) return;
    ...
    fetchCart()
      .then(function (cart) {
        ...
        return cartChange(item.key, 0).then(function () {
          if (bundleId && !item.properties._pdp35_gift) return reconcileBundle(bundleId);
